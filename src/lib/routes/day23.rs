...
pub async fn day23_task3(Path(color): Path<String>) -> impl IntoResponse {
    ...
    let safe_color = encode_text(&color);
    if let Some(&next_color) = color_map.get(safe_color.as_ref()) {
        let html = format!(
...
pub async fn day23_task4(Path((state, n)): Path<(String, String)>) -> impl IntoResponse {
    let safe_state = encode_text(&state);
    let safe_n = encode_text(&n);
    
    if safe_state != "on" && safe_state != "off" {
        return (StatusCode::IM_A_TEAPOT).into_response();
    }

    let next_state = if safe_state == "on" { "off" } else { "on" };
    ...
    let html = format!(
      ...
      "#,
        class, safe_n, next_state, safe_n
    );
